...
export default function IndicatorOptimizer({ mode, defs, entry, tf, appSyms, currentSl, currentTp, onApply }) {
  ...
  const run = async (obj) => {
    setObjective(obj);
    if (!entry || !entry.length || !appSyms || !appSyms.length || !numeric) {
  ...
  const optBtn = (k, label) => (
    <button key={k} onClick={() => run(k)} disabled={loading || !numeric} title={!numeric ? "This screener has no tunable indicator lengths" : undefined} className="tap disp" style={{
    ...
    <div style={{ marginTop: 12 }}>
      ...
      {ran && !loading && (!best) && (
        <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 8, lineHeight: 1.5 }}>
          {!numeric
            ? "This screener has no tunable indicator lengths to optimise — add an indicator with a length (EMA, RSI, Bollinger…)."
      ...
      {ran && !loading && best && (
        <div style={{ marginTop: 10, border: "1px solid var(--line)", borderRadius: 11, padding: 11, background: "var(--surface)" }}>
          ...
          {/* Per-indicator change: name · length@tf → length@tf */}
          <div style={{ fontSize: 10.5, color: "var(--ink)", marginTop: 8, lineHeight: 1.5 }}>
            {changes.length
              ...
              : `Current indicators are already optimal @ ${best.tf}.`}
          </div>
          ...
          {onApply && changes.length > 0 && (
            ...
            >
              {applied ? "✓ Applied tuned indicators" : `Apply tuned indicators @ ${best.tf}`}
            </button>
          )}
        </div>
      )}
