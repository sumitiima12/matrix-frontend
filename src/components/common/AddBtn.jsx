...
export default function AddBtn({ on, onClick, size = 28 }) {
  return (
    <button onClick={(e) => { e.stopPropagation(); onClick(); }} className="tap" title={on ? "In watchlist" : "Add to watchlist"}
      style={{ width: size, height: size, borderRadius: 9, border: "1px solid " + (on ? "var(--primary)" : "var(--line)"), background: on ? "var(--primary)" : "var(--elev)", color: on ? "var(--on-primary)" : "var(--primary)", display: "grid", placeItems: "center", flex: "0 0 auto" }}>
