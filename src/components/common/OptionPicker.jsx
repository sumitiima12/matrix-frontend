...
/**
 * OptionPicker — choose a real option contract.
 *
 * Every choice here is constrained to what the BROKER says exists. The expiry list is
 * the broker's. The strikes are the broker's. The symbol is the broker's. Nothing is
 * assembled from parts, because a symbol assembled from a wrong expiry rule or a wrong
 * strike interval still LOOKS like a symbol — and the exchange will happily fill it on
 * a contract you never intended to trade.
 *
 * The resolved contract is shown in full before you can confirm. You should always be
 * able to see exactly what you are about to buy.
 */
export default function OptionPicker({ underlying, spot, userId, onPick }) {
  ...
  // The spot the STRIKE is measured against. Prefer the broker's own spot: it is the
  // number their chain is centred on. Fall back to our live price only if they omit it.
  ...
  /* No chain -> no options. We say why, and we stop. The alternative — building a
     symbol ourselves — is the one thing we will not do. */
  if (error || !chain) {
    return (
  ...
  return (
    <div style={{ marginTop: 12 }}>
      {/* EXPIRY — the broker's real dates. We label the nearest and the front-month one,
          but the actual date is always shown: an expiry rule can go stale, a date can't. */}
      ...
      <div style={{ display: "flex", gap: 7 }}>
        {[["CE", "CALL"], ["PE", "PUT"]].map(([k, l]) => (
          <button key={k} onClick={() => setType(k)} className="tap disp"
            style={{ ...pill(type === k), flex: 1, background: type === k ? (k === "CE" ? "var(--up)" : "var(--down)") : "var(--surface)", borderColor: type === k ? (k === "CE" ? "var(--up)" : "var(--down)") : "var(--line)" }}>
            {l}
          </button>
        ))}
      </div>

      {/* MONEYNESS — ITM/OTM mean OPPOSITE strike directions for calls and puts. The
          hint below spells out which way, because getting it backwards buys the inverse
          of your thesis. */}
      ...
      {moneyness !== "ATM" && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <button onClick={() => setSteps((s) => Math.max(1, s - 1))} className="tap" style={{ width: 28, height: 28, borderRadius: 8, border: "1px solid var(--line)", background: "var(--elev)", color: "var(--ink)", fontWeight: 800, cursor: "pointer" }}>–</button>
            <span className="mono" style={{ fontWeight: 800, minWidth: 18, textAlign: "center" }}>{steps}</span>
            <button onClick={() => setSteps((s) => Math.min(20, s + 1))} className="tap" style={{ width: 28, height: 28, borderRadius: 8, border: "1px solid var(--line)", background: "var(--elev)", color: "var(--ink)", fontWeight: 800, cursor: "pointer" }}>+</button>
          </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <button onClick={() => setLots((l) => Math.max(1, l - 1))} className="tap" style={{ width: 34, height: 34, borderRadius: 9, border: "1px solid var(--line)", background: "var(--elev)", color: "var(--ink)", fontWeight: 800, fontSize: 16, cursor: "pointer" }}>–</button>
        <span className="mono" style={{ fontWeight: 800, fontSize: 15, minWidth: 26, textAlign: "center" }}>{lots}</span>
        <button onClick={() => setLots((l) => Math.min(50, l + 1))} className="tap" style={{ width: 34, height: 34, borderRadius: 9, border: "1px solid var(--line)", background: "var(--elev)", color: "var(--ink)", fontWeight: 800, fontSize: 16, cursor: "pointer" }}>+</button>
      ...
      {/* THE RESOLVED CONTRACT. Shown in full, always. You should never confirm an order
          without seeing the exact instrument it will hit. */}
        ...
        {!contract ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 14, marginTop: 8, flexWrap: "wrap", fontSize: 11.5, color: "var(--muted)" }}>
              ...
              <span>Spot <b style={{ color: "var(--ink)" }}>{px != null ? fmt(px, "FNO") : "—"}</b></span>
              <span>Expiry <b style={{ color: "var(--ink)" }}>{expiryLabel(contract.expiry)}</b></span>
              {contract.ltp != null && <span>Premium <b style={{ color: "var(--ink)" }}>{fmt(contract.ltp, "FNO")}</b></span>}
            </div>
            {contract.ltp != null && qty != null && (
              <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 6 }}>
                Order value ≈ <b style={{ color: "var(--ink)" }}>{fmt(contract.ltp * qty, "FNO")}</b> ({lots} lot{lots > 1 ? "s" : ""} × {lotSize})
              </div>
            )}
          </>
        )}
      ...
      <button
        ...
        style={{
          ...
          background: contract && qty != null ? (type === "CE" ? "var(--up)" : "var(--down)") : "var(--line)",
          color: "#fff", fontWeight: 800, fontSize: 14,
          cursor: contract && qty != null ? "pointer" : "not-allowed",
        }}
      >
        {contract && qty != null ? `Buy ${lots} lot${lots > 1 ? "s" : ""} · ${moneyness} ${type === "CE" ? "CALL" : "PUT"}` : "Select a valid contract"}
      </button>
    </div>
  );
}
