...
function AutoExitsCard({ userId, heldSyms = null }) {
  ...
  useEffect(() => { refresh(); const id = setInterval(refresh, 300000); return () => clearInterval(id); /* eslint-disable-next-line */ }, [userId]);   // 5-min poll — portfolio/auto-exits status doesn't need frequent refresh
...
function HoldingIntel({ a, market = "IN", stock, onWhy }) {
  ...
  if (!a) return null;
  const col = a.action === "Add" || a.action === "Take profit" ? "var(--up)" : a.action === "Exit" ? "var(--down)" : a.action === "Reduce" ? "#F59E0B" : "var(--muted)";
  if (!a.hasData) {
    ...
    return (
      <div style={{ padding: "6px 0 20px" }}>
        ...
        {!hold.length ? (
          ...
          // analyzer a positive qty + an explicit short flag so its P&L%/trend read the right direction.
          const anaShort = h.short || h.side === "SELL" || Number(h.qty) < 0;
